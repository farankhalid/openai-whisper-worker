...
# whisper ${1} --language ${2} --model_dir ${3} --model large --task transcribe --device gpu --output_dir ${} --threads 2
...
if [ "$env" = "prod" ]; then
    /home/ubuntu/quick-translates-api-worker-env/bin/whisper "$file_name" \
      --language "$language" \
      --model_dir "$model_dir" \
      --model "$model" \
      --task "$job" \
    ...
    /home/faran/PycharmProjects/quick-translates-api-worker/venv/bin/whisper "$file_name" \
      --language "$language" \
      --model_dir "$model_dir" \
      --model "$model" \
      --task "$job" \
